...
| **Index & search** | `reindex`, `symbol_search`, `code_search` (NL intent search), `repo_map` (98ms file map) |
...
25+ rounds of "Malong reviews Malong": the toolset audits and fixes its **own** code, with dozens of real bugs fixed and locked by tests — including directory-scope filtering gaps, false dead-code reports for registration patterns, lost constant read-sites, and SQL parameterization cleanup. Every round grew the assertion count (now 1874 across 81 JS test files, full chain 0 failures).
...
cd malong
npm ci
npm test          # 1874 assertions (daemon must be running)
```
