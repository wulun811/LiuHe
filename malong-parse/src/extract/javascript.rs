...
pub fn extract_variable_refs(tree: &Tree, source: &str) -> Vec<Reference> {
    ...
    fn walk_collect(node: Node, source: &str, depth: u32, out: &mut Vec<String>) {
        if depth > super::MAX_WALK_DEPTH { return; }
        collect_top_level_bindings(node, source, out);
        for i in 0..node.child_count() {
            if let Some(c) = node.child(i) {
                walk_collect(c, source, depth + 1, out);
            }
        }
    }
    walk_collect(tree.root_node(), source, 0, &mut top_bindings);
    ...
    fn walk(node: Node, source: &str, depth: u32, top: &[String], refs: &mut Vec<Reference>) {
        if depth > super::MAX_WALK_DEPTH { return; }
        ...
        for i in 0..node.child_count() {
            if let Some(c) = node.child(i) {
                walk(c, source, depth + 1, top, refs);
            }
        }
    }
    walk(tree.root_node(), source, 0, &top_bindings, &mut refs);
    refs
}
